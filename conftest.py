...
def driver(request):
    ...
    driver.get(handlers.Handlers.URL)
    yield driver
    driver.quit()
...
def generate_user_register_and_delete_by_api():
    ...
    registration_response = requests.post(f'{handlers.Handlers.URL + handlers.Handlers.CREATE_USER}', json=payload)
    registration_info = []
    ...
    login_response = requests.post(f'{handlers.Handlers.URL + handlers.Handlers.LOGIN_USER}', json=payload)
    token = login_response.json()['accessToken']
    headers = {
        "Authorization": token
    }
    requests.delete(f'{handlers.Handlers.URL + handlers.Handlers.INFO_USER}', headers=headers)